...
* A team's rate of rush shots that were preceded by passes is not repeatable within seasons
...
* Rebound shots are not repeatable over a given season. Furthermore, rebound shooting percentages (rebound goals _(goals scored from rebounds)_ divided by rebound shots) are repeatable.
...
jason_theme <- theme_bw() + theme(text=element_text(family="Times"))
theme_set(jason_theme)
...
# Plot
 pass_shooting_percentage_zone %>%
   ...
   geom_smooth(method = "lm") +
   facet_wrap(~a1_zone, labeller = pass_lanes_labeller) +
   scale_color_discrete(name = "Seasons",
                       breaks = c("first", "second", "third"),
                       labels = c("2015-2016", "2016-2017", "2017-2018")) +
   theme(text=element_text(family="Times", size = 16)) +
...
pass_final %>% 
  ...
  ggtitle("Distribution of Rebound Shots on Goal", subtitle = "Horizontal yellow line indicates 42 rebound shots on goal or one per game") 
```
